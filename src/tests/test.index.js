...
describe('test ethjs-format object', () => {
  ...
  describe('test decode of getBlockByNumber', () => {
    ...
    const formattedData = format.formatOutputs('platon_getBlockByHash', payload);
  ...
  describe('test formatInputs', () => {
    it ('should handle platon_getBalance normally', () => {
      const encodedBalance = format.formatInputs('platon_getBalance', ['0xb60e8dd61c5d32be8058bb8eb970870f07233155']);

      assert.equal(encodedBalance, '0xb60e8dd61c5d32be8058bb8eb970870f07233155');
    });

    it ('should handle platon_getBalance 2 arguments normally', () => {
      const encodedBalance = format.formatInputs('platon_getBalance', ['0xb60e8dd61c5d32be8058bb8eb970870f07233155', 'latest']);

      assert.equal(encodedBalance[0], '0xb60e8dd61c5d32be8058bb8eb970870f07233155');
      assert.equal(encodedBalance[1], 'latest');
    });

    it ('should handle platon_sendTransaction normally', () => {
      const encodedSendTransaction = format.formatInputs('platon_sendTransaction', [{
        'from': '0xb60e8dd61c5d32be8058bb8eb970870f07233155',
        'to': '0xd46e8dd67c5d32be8058bb8eb970870f07244567',
        'gas': new BN('30400'), // 30400,
        'gasPrice': '10000000000000', // 10000000000000
  ...
  describe('test formatOutputs', () => {
    it ('should handle invalid platon_getTransactionByBlockHashAndIndex normally', () => {
      try {
        const decodedHashRate = format.formatInputs('platon_getTransactionByBlockHashAndIndex', ['0x']);
      } catch (error) {
        assert.equal(typeof error, 'object');
      }
    });

    it ('should handle platon_getFilterChanges normally during platon_newBlockFilter', () => {
      const decodedTxHashArray = format.formatOutputs('platon_getFilterChanges', ['0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55', '0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55']);
    ...
    it ('should handle platon_getFilterChanges normally during platon_newPendingTransactionFilter', () => {
      const decodedTxHashArray = format.formatOutputs('platon_getFilterChanges', ['0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55', '0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55']);

      assert.equal(decodedTxHashArray.toString(10), ['0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55', '0xdf829c5a142f1fccd7d8216c5785ac562ff41e2dcfdf5785ac562ff41e2dcf55']);
    });

    it ('should handle platon_hashrate normally', () => {
      const decodedHashRate = format.formatOutputs('platon_hashrate', '0x38a');

      assert.equal(decodedHashRate.toString(10), '906');
    });

    it ('should handle platon_getBalance normally', () => {
      const decodedBalance = format.formatOutputs('platon_getBalance', '0x38a');

      assert.equal(decodedBalance.toString(10), '906');
    });

    it ('should handle empty platon_hashrate normally', () => {
      const decodedHashRate = format.formatOutputs('platon_hashrate', '0x');

      assert.equal(decodedHashRate.toString(10), '0');
    });

    it ('should handle platon_mining true normally', () => {
      const decodedMining = format.formatOutputs('platon_mining', true);

      assert.equal(decodedMining, true);
    });

    it ('should handle platon_mining false normally', () => {
      const decodedMining = format.formatOutputs('platon_mining', false);

      assert.equal(decodedMining, false);
    });

    it ('should handle platon_accounts normally', () => {
      const decodedEthAccounts = format.formatOutputs('platon_accounts', ['0x407d73d8a49eeb85d32cf465507dd71d507100c1', '0x407d73d8a49eeb85d32cf465507dd71d507100c1', '0x407d73d8a49eeb85d32cf465507dd71d507100c1']);
    ...
    it ('should handle empty platon_accounts normally', () => {
      const decodedEthAccounts = format.formatOutputs('platon_accounts', []);

      assert.equal(decodedEthAccounts.length, 0);
    });

    it ('should handle platon_getFilterChanges normally', () => {
      const decodedFilterChanges = format.formatOutputs('platon_getFilterChanges', [{
        'logIndex': '0x1', // 1
        'blockNumber':'0x1b4', // 436
